...
function extractMeta(source) {
    ...
    if (!parsed || parsed.length === 0) {
        debug_1.usercssmeta(chalk_1.default.ansi256(208) `Failed to match usercssmeta with capture regex.`);
        return '';
    }
    debug_1.usercssmeta(chalk_1.default.blue `Extracted meta:\n` + chalk_1.default.gray(parsed));
    return parsed;
}
...
function parseSourceUserCSSMeta(source, allowErrors = true) {
    ...
    if (extracted.length === 0) {
        debug_1.usercssmeta(chalk_1.default.ansi256(209) `Failed to extract meta, exiting parse function.`);
        return;
    }
